...
今後このブランチで作業をし、このブランチと同名のリモートブランチにプッシュし、報告する事で課題提出とします。  
...
チームはアジャイル開発を取り入れており、スプリント単位は２週間です。  
翌スプリントの施策をKPIや議事録を元にいくつか好きに考えて、その施策の実装を実際に２週間で行いなさい。

#### サービスの状況
 
【サービス概要】  
タレントがチャンネルを持ち、縦型短尺動画コンテンツを配信していくサービス。  
...
・翌日RR / 7日後RR が33%と高めなのは、潜在的に長期滞在してくれるコアファン以外が初日にほとんど落ちてるから（？）
・デザイン的に、動画サービスとしてのブランディングがなされていない
```
