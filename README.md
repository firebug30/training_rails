...
#### クローン
以下のコマンドでこのレポジトリをローカルにクローンします。

```sh
git clone https://github.com/firebug30/training_rails.git
...
#### 作業用ブランチを作成 & チェックアウト
masterブランチでクローンされると思うので、自分の作業用のブランチを作成してチェックアウトします。  
今後このブランチで作業をし、このブランチと同名のリモートブランチにプッシュし、報告する事で課題提出とします。  

```sh
git checkout -b feature/20180101/keiji_namba

##############################################
#
# 【ブランチ名について】
# リモートブランチにも同名のものを作成する事
# 
# feature/{作業開始日}/{名前_苗字}
#
##############################################
...
```sh
pwd
#=> /..{your working directory}../training_rails

# .envファイルの作成して以下の内容で保存
#
...
あなたは、以下のプロジェクトのチームに所属しています。  
チームはアジャイル開発を取り入れており、スプリント単位は２週間です。  
翌スプリントの施策をKPIや議事録を元にいくつか好きに考えて、その施策の実装を実際に２週間で行いなさい。

#### サービスの状況
 
【サービス概要】  
タレントがチャンネルを持ち、縦型短尺動画コンテンツを配信していくサービス。  
...
```
・MAUは直近KPIにもう少し
・MAUの成長曲線的にも悪くなく、数スプリント以内に目標の５万人に達成しそう
・DAU / MAU のアクティブ率がかなり低い
・翌日RRが30%と低い
・7日後RRはほぼ成長しておらず、10%程度
・翌日RR / 7日後RR が33%と高めなのは、潜在的に長期滞在してくれるコアファン以外が初日にほとんど落ちてるから（？）
・デザイン的に、動画サービスとしてのブランディングがなされていない
```

### 提出時に埋める

提出時は、上記に則って以下を埋めてください。(README.md)
